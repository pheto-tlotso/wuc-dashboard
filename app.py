...
with st.sidebar:
    ...
    st.divider()
    st.markdown("**Prototype v1.0 — Rooteddeck**")
...
<div style='background:#0B1F3A;padding:1rem 1.5rem;
  ...
  <p style='color:#AACCE8;margin:0.3rem 0 0;font-size:0.9rem'>
    Prototype · Rooteddeck Analytics
  </p>
</div>
...
# ── Global NRW header ─────────────────────────────────────────────
try:
    billing = load_csv_from_s3('data/clean/billing_features.csv')
    for col in ['nrw','nrw_revenue_loss_BWP','nrw_revenue_loss_bwp','rev']:
    ...
    avg_nrw      = billing_filt['nrw'].mean() * 100
    ...
    worst_zone   = (billing_filt.groupby('zone_id')['nrw']
                    .mean().idxmax()
                    if len(billing_filt) > 0 else "N/A")

    h1,h2,h3,h4,h5 = st.columns(5)
    h1.metric("Avg NRW %",       f"{avg_nrw:.1f}%",
              delta=f"{avg_nrw-35:.1f}% vs 35% benchmark",
              delta_color="inverse")
    h2.metric("Revenue (sample)", f"P{total_loss/1e6:.1f}M")
...
with tab1:
    ...
    try:
        iot_raw     = load_csv_from_s3('data/clean/iot_features.csv')
        ...
        if 'hour' not in iot_tod.columns:
            if 'timestamp' in iot_tod.columns:
                ...
                st.warning("No timestamp or hour column found.")
                iot_tod = None
        ...
        if iot_tod is not None:
            ...
            m1.metric("2–4 AM Leaks (MNF Window)", mnf_leaks)
            m2.metric("6–9 AM Leaks (Morning Surge)", morning_leaks)
            m3.metric(
        ...
        if iot_tod is not None:
            ...
            st.dataframe(
                zone_display.sort_values('mnf_leak_count', ascending=False)
...
with tab2:
    ...
    try:
        quality_raw  = load_csv_from_s3('data/clean/iot_features.csv')
        ...
        chl_col = next((c for c in ['chlorine_residual','chlorine_mg_L','chlorine_mg_l']
                        if c in quality.columns), None)
        ph_col  = next((c for c in ['ph_level','pH','ph']
                        if c in quality.columns), None)
        tur_col = next((c for c in ['turbidity','turbidity_NTU','turbidity_ntu']
                        if c in quality.columns), None)
        ...
        if chl_col and ph_col:
            ...
            fig2 = px.scatter(
                ...
                labels={chl_col:'Chlorine', ph_col:'pH'},
            )
            fig2.update_layout(height=380, plot_bgcolor='rgba(0,0,0,0)')
            st.plotly_chart(fig2, use_container_width=True)

        top_feat = xgb_metrics.get('top_feature','pressure_bar')
...
# ══════════════════════════════════════════════════════════════════
with tab3:
    st.subheader("Pipe Risk Map — XGBoost Model")
    try:
        from streamlit_folium import st_folium
        import folium
        from folium.plugins import MarkerCluster

        pipes_raw  = load_csv_from_s3('data/clean/pipe_risk_scores.csv')
        lr_metrics = load_metrics_from_s3('warm/models/logreg_pipe_risk_metrics.json')

        for col in ['latitude','longitude','prob_low','prob_medium',
        ...
        pipes = pipes_raw[
            ...
            pipes_raw['latitude'].notna() &
            pipes_raw['longitude'].notna() &
            (pipes_raw['latitude'].abs()  > 0.01) &
            (pipes_raw['longitude'].abs() > 0.01)
        ].copy()
        ...
        k4.metric("F1-Macro",       f"{lr_metrics.get('f1_macro',0):.4f}")
        st.divider()
        ...
        from src.feature_utils import safe_map_centre
        centre_lat, centre_lon = safe_map_centre(pipes, 'latitude', 'longitude')
...
with tab4:
    ...
    try:
        fc_raw    = load_csv_from_s3('data/clean/demand_forecasts_6month.csv')
        bill_raw  = load_csv_from_s3('data/clean/billing_features.csv')
        ...
        k1,k2,k3,k4 = st.columns(4)
        k1.metric("Mean MAPE",  f"{mean_mape:.2f}%")
        ...
        if len(future_fc) > 0:
            ...
            trough_row = future_fc.loc[future_fc['yhat'].idxmin()]
            st.info(
                f"**Forecast Insight:** Peak demand of "
                f"{peak_row['yhat']:,.0f} m³ forecast for zone "
                f"**{peak_row['zone_id']}** on "
                f"{pd.to_datetime(peak_row['ds']).strftime('%d %b %Y')}. "
                f"Lowest demand of {trough_row['yhat']:,.0f} m³ expected "
                f"for **{trough_row['zone_id']}** on "
                f"{pd.to_datetime(trough_row['ds']).strftime('%d %b %Y')}."
            )
